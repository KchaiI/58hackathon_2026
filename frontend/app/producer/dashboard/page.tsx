...
export default function ProducerDashboard() {
  ...
  useEffect(() => {
    fetch(`${process.env.NEXT_PUBLIC_API_URL}/producers/`)
      .then((res) => res.json())
      .then((data) => {
  ...
  useEffect(() => {
    ...
    fetch(
      `${process.env.NEXT_PUBLIC_API_URL}/listings/?producer_id=${selectedId}`,
    )
      .then((res) => res.json())
      .then((data) => {
        setListings(data ?? []);
  ...
  return (
    <main className="w-full px-12 py-8">
      ...
      {/* 生産者セレクター */}
      <div className="mb-6">
        <label className="block text-sm font-medium text-[#5a5040] mb-2">
          生産者を選択
        </label>
        <select
          value={selectedId}
        ...
        >
          <option value="">
            {producers.length === 0
              ? "読み込み中..."
              : `-- 生産者を選んでください (${producers.length}件) --`}
          </option>
          {producers.map((p) => (
      ...
      {selectedProducer && (
        <div className="border border-[#e0dbd2] rounded-2xl p-6">
          ...
          {loadingListings ? (
            <p className="text-[#9a9080] text-center py-10 text-sm">
              読み込み中...
            </p>
          ) : listings.length === 0 ? (
            ...
            <div className="space-y-3">
              {listings.map((listing) => {
                const sold = listing.total_slots - listing.available_slots;
                const pct =
                  listing.total_slots > 0
                    ? Math.round((sold / listing.total_slots) * 100)
                    : 0;
                    ...
                    <div className="flex gap-4">
                      ...
                      <div className="flex-1 min-w-0">
                        ...
                        <div className="flex justify-between items-start mb-3">
                          ...
                          <span
                            className={`text-xs px-3 py-1 rounded-full font-medium shrink-0 ${status.cls}`}
                          >
                            {status.label}
                          </span>
                        </div>
                        ...
                        <ul className="space-y-1">
                          {owners.map((o, i) => (
                            ...
                            >
                              ...
                              <span className="text-[#9a9080] text-xs truncate">
                                {o.owner_email}
                              </span>
                            </li>
                          ))}
